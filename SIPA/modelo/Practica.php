...
//
    ...
    function select($argumentos) {
        ...
        $count = UtilConexion::$pdo->query("SELECT id_practica FROM practica $where")->rowCount();
        ...
        $sql = "SELECT * FROM practica $where";// ORDER BY $sidx $sord LIMIT $rows OFFSET $start";
        foreach (UtilConexion::$pdo->query($sql) as $fila) {
            $respuesta['rows'][] = [
                'id' => $fila['id_practica'],
                'cell' => [$fila['id_practica'], $fila['fecha_inicio'],$fila['fecha_fin'],$fila['salario'],$fila['fk_convenio']]
            ];
        }
